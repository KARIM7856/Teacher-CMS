...
GUEST_USERNAME=guest                     # optional; blank hides the guest button
GUEST_PASSWORD=...                       # optional
LEGAL_BASE_URL=...                       # optional; defaults to the Vercel admin URL
```

Without credentials the app launches to a "not configured" screen. With them you
get the sign-in screen — there is no sign-up, since the teacher creates every
account (see `/CLAUDE.md` → Phase 7). Sessions persist across launches.
...
| Launcher / display name | منصة المعلّم |
| Version | `pubspec.yaml` → `version:` (name`+`build number) |

Both identifiers are **permanent** once either store accepts an upload. Icons for
every density and both platforms are generated by
`node tools/generate_app_icons.mjs` from the repo root — don't hand-edit them.

To publish, follow [`/RELEASE.md`](../RELEASE.md); listing copy and console
answers are in [`/store`](../store).
